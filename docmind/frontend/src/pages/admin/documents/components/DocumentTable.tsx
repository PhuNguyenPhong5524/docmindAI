...
export const DocumentTable: React.FC<DocumentTableProps> = ({
  ...
  const getInitials = (name: string) => {
    if (!name || name.startsWith("User:")) return "U"; // Thấy chữ User: là gán thành U luôn cho gọn
        ...
        <table className="w-full text-left text-xs border-collapse min-w-[820px]">
          ...
          <tbody className="divide-y divide-slate-100">
            {documents.length === 0 ? (
              ...
              documents.map((doc) => {
                const realName = doc.ownerName || "Hệ thống";
                return (
                  <tr key={doc.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex flex-col min-w-0 max-w-[180px]">
                          <span className="font-medium text-xs text-slate-900 truncate" title={realName}>
                            {realName}
                          </span>
                    ...
                    {/* Lấy đúng size dạng chữ truyền xuống, hết NaN */}
